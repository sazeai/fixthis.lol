...
export function PostProblemModal({
  ...
  const [success, setSuccess] = useState<"published" | "pending" | "">("")
  ...
  const triggerClass = asLink
    ...
    : inverted
    ? "inline-flex min-h-12 items-center gap-2 rounded-full bg-white px-6 text-[13px] font-bold uppercase tracking-[0.08em] text-[#111] shadow-[0_4px_18px_rgba(255,255,255,.13)] transition hover:-translate-y-0.5 hover:bg-[#fafafa]"
    : compact
      ? "inline-flex min-h-9 items-center gap-2 rounded-full bg-[#111] px-4 text-[10px] font-bold uppercase tracking-[0.08em] text-white transition-colors hover:bg-[#ef4e37]"
      : "inline-flex min-h-11 items-center gap-2 rounded-full bg-[#111] px-6 text-[12px] font-bold uppercase tracking-[0.08em] text-white shadow-[0_8px_24px_rgba(0,0,0,.14)] transition hover:-translate-y-0.5 hover:bg-[#ef4e37]"

  return <>
    <button type="button" onClick={() => { setOpen(true); setSuccess(""); setError("") }} className={triggerClass}>
      {trigger}{asLink ? null : <ArrowRight size={14} />}
    </button>
      ...
      {success ? (
        ...
        <div className="px-6 pb-9 pt-3 sm:px-8">
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            ...
            >
              <TextInput name="email" type="email" maxLength={254} placeholder="you@company.com" />
            </FormField>
            ...
            <button disabled={loading} className="flex h-12 w-full items-center justify-center gap-2 bg-[#111] text-[11px] font-bold uppercase tracking-[0.1em] text-white transition-colors hover:bg-[#ef4e37] disabled:opacity-60">
              {loading ? <><LoaderCircle className="animate-spin" size={15} /> Publishing…</>
                : <>{founder ? "Add problem & continue" : "Publish problem"} <ArrowRight size={14} /></>}
            </button>
          </form>
        </div>
      )}
