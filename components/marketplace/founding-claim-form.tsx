...
export function FoundingClaimForm({ problems }: { problems: Array<{ id: string; statement: string }> }) {
  ...
  return (
    <form ref={formRef} onSubmit={submit} className="grid gap-3 border border-[rgba(55,50,47,0.12)] bg-white p-5 md:grid-cols-2">
      ...
      <button disabled={loading} className="flex h-11 items-center justify-center gap-2 bg-[#111] text-[10px] font-bold uppercase tracking-[0.1em] text-white transition-colors duration-200 ease-out hover:bg-[#ef4e37] active:scale-[0.99] disabled:opacity-60 md:col-span-2">
